...
Esgi::Mat4 LookAtCamera(float px, float py, float pz, float tx, float ty, float tz)
{
	//px, py et pz sont les coordonnées X Y et Z de la caméra
	//tx, ty et tz sont les coordonnées X Y et Z du Target du LookAt

	float vpX, vpY, vpZ, vtX, vtY, vtZ, cosy, cosx, angleY, angleX;
	vpX = 0;
	vpY = 0;
	vpZ = 1;
	vtX = tx - px;
	...
	cosy = (vpX * vtX + vpZ * vtZ) / sqrt(vtX*vtX + vtZ*vtZ);
	cosx = (vpY * vtY + vpZ * vtZ) / sqrt(vtY*vtY + vtZ*vtZ);
	...
	tran.MakeTranslation(px, py, pz);

	rotX.MakeRotation3DY(angleY);
	rotY.MakeRotation3DX(angleX);

	LookAt.Identity();

	LookAt =  rotY * rotX * tran;
